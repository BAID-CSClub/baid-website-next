<template>
  <div class="section">
    <div class="flex items-center justify-between">
      ...
      <router-link class="flex items-center gap-1 color-black decoration-none op-80 hover:op-100 transition"
        :to="'/' + $i18n.locale + '/news'">
    ...
    <div class="flex h-130">
      <div class="flex-1 relative b-1 b-black b-solid b-r-0 m-t--1px m-b--1px">
        <div class="transition-opacity-300 h-full cursor-pointer" :class="{ 'op-0': transition }"
          @click="router.push(props.newsList[current].href)">
          <img :src="props.newsList[current].cover" alt="" class="w-full h-full object-cover" />
      ...
      <div class="w-85 flex flex-col">
        <div class="h-25% flex b-1 b-black b-solid transition-all-300 bg-white m-t--1px m-b--1px" :class="{
          'scale-107 shadow-lg !bg-[var(--standard-red)] border-color-[var(--standard-red)]':
            current === index && !transition
        }" v-for="(news, index) in props.newsList.slice(0, 4)" :key="index" v-on:click="change(index)">
          <div class="w-50% ml-3 mt-3">
            <p class="font-sans color-gray-8 text-4 mb-0 transition-color-300"
              :class="{ '!color-gray-2': current === index && !transition }">
              {{ news.date.toLocaleString().substring(0, 10) }}
            </p>
            <h3 class="my-1 font-sans color-[var(--standard-blue)] text-6 transition-color-300" style="
                overflow: hidden;
                display: -webkit-box;
          ...
          <img :src="news.cover" class="w-50% transition-all-300 object-cover aspect-4/3"
