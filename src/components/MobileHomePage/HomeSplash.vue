<template>
  <div class="relative" :class="{ 'h-screen overflow-hidden': show }">
    <slot></slot>
    <div class="bg-blue fixed top-0 w-full h-full transition-opacity-300 z-1"
      :class="{ 'op-0 pointer-events-none': !show }">
      <!-- Foreground -->
      <div class="w-full h-90% relative" @touchmove="onMove" @touchstart="onStart">
        <!-- BG -->
        <Swiper class="h-full brightness-80" @slide-change="change($event.activeIndex)" @swiper="swiper = $event"
          :resistance-ratio="0">
          <SwiperSlide v-for="img in pageData.carousel_images" :key="img">
            <img :src="img" class="w-full h-full object-cover" alt="" srcset="" />
          </SwiperSlide>
        </Swiper>

        <!-- Content -->
        <div class="border-l-3 p-l-3 border-white absolute bottom-10 left-10% color-white w-70% z-1">
        ...
        <div class="absolute bottom-3 left-50% translate-x--50% flex z-1">
          <div v-for="(_, index) in pageData.carousel_images" :key="index"
            class="border-solid border-white rounded-50% w-3 h-3 m-x-1" :class="{ 'bg-white': index === current }"
            @click="change(index)"></div>
        </div>
      </div>
      <div class="text-center font-sans color-white absolute bottom-0 left-50% translate-x--50%" @click="show = false">
