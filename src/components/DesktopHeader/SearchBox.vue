<template>
  <div>
    <div type="text" placeholder="Search"
      class="w-5 h-7 ml-2 mr-10 op-50 hover:op-100 active:op-80 transition color-inherit" :style="{ color: props.color }"
    ...
    <div v-show="showSearchBox"
      ...
      @click="toggleSearchBox()">
      <div class="bg-white p-5 max-w-150 max-h-70% overflow-auto" @click.stop="() => { }">
        ...
        <div class="p-3" v-show="text.length > 0">
          <RouterLink v-for="(result, index) in results" :key="result.ref" :to="db[locale][result.ref].href"
