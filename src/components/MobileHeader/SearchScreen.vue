<template>
  <div class="fixed top-0 w-full h-screen bg-[var(--standard-red)] transition-all-300 z-4 px-10 py-6 box-border" :class="{
    'left-full': !props.show,
    'left-0': props.show
  }">
    <input v-model="text" type="text" placeholder="搜索内容"
    ...
    <Transition mode="out-in" name="fade">
      <div class="mt-15 color-white transition-opacity" v-if="results.length">
        ...
        <RouterLink class="block bg-white bg-op-10 w-full rounded-xl pa-5 box-border !decoration-none mb-5"
          @click="emit('close')" v-for="result in results" :key="result.ref" :to="db[locale][result.ref].href">
